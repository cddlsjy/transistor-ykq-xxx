...
public final class CollectionAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements TransistorKeys {
    ...
    public interface CollectionAdapterListener {
        ...
        void itemFocused(Station station);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        // CASE STATION
            ...
            // 焦点变化监听 - 用于电视遥控器上下键切换
            stationViewHolder.getListItemLayout().setOnFocusChangeListener(new View.OnFocusChangeListener() {
                @Override
                public void onFocusChange(View view, boolean hasFocus) {
                    if (hasFocus) {
                        // 当项获得焦点时，通知监听器
                        int focusedPosition = holder.getAdapterPosition();
                        if (focusedPosition >= 0 && focusedPosition < mStationList.size()) {
                            mCollectionAdapterListener.itemFocused(mStationList.get(focusedPosition));
                        }
                    }
                }
            });
    ...
    /* 设置选中电台的ID并更新视图 */
    public void setSelectedStationId(int position) {
        if (position != mStationIdSelected) {
            // 取消上一个选中项的高亮
            notifyItemChanged(mStationIdSelected, HOLDER_UPDATE_SELECTION_STATE);
            // 更新选中项
            mStationIdSelected = position;
            // 高亮新的选中项
            notifyItemChanged(position, HOLDER_UPDATE_SELECTION_STATE);
        }
    }
